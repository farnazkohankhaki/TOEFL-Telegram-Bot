 ...
 */
public class InstitueState implements BotState
{
    private long chatId;
    MainController mainController;
    public InstitueState(MainController mainController, long chatId)
    {
        this.chatId = chatId;
    ...
    public void validate(Update update)
    {
        if(update.getMessage().getText().length() < 30)
        {
            ...
            List<InstitueModel> institueModelList = institue.getAll();
            String message = "";
            for (int i = 0; i < institueModelList.size(); i++)
            {
                InstitueModel institueModel = institueModelList.get(i);
                message += "Institue Name: " + institueModel.getName() + "\n" + "Phone Number: " + institueModel.getPhone() + "\n" + "Address: " + institueModel.getAddress() + "\n\n";
            }
            String keyboardMessage = "Ok";
            this.changeState("institue");
