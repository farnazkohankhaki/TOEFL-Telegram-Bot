...
public class AdminWaitingState implements BotState
{

    private long chatId;
    MainController mainController;
    public AdminWaitingState(MainController mainController, long chatId)
    {
        this.chatId = chatId;
    ...
    public void validate(Update update)
    {
        if(update.getMessage().getText().length() < 30)
        {
            ...
            String keyboardMessage = "Add Word,Back to Home";
            this.changeState("adminWaiting");
            this.response(message, keyboardMessage);
        }
    }

    public void validate(Update update, String vocab, String meaning)
    {
        if(update.getMessage().getText().length() < 30)
        {
            Word word = new Word();
            WordModel wordModel = new WordModel(vocab, meaning);
            word.insert(wordModel);
            String message = "Choose a Service";
            String keyboardMessage = "Add Word,Back to Home";
            this.changeState("adminWaiting");
            this.response(message, keyboardMessage);
